...
class YTVOSeval:
    ...
    def _prepare(self):
        ...
        def _toMask(anns, coco):
            ...
            for ann in anns:
                if 'segmentations_refined' in ann.keys():
                    for i, a in enumerate(ann['segmentations_refined']):
                        if a:
                            rle = coco.annToRLE(ann, i)
                            ann['segmentations_refined'][i] = rle
                else:
    ...
    def computeBoundaryIoU(self, vidId, catId):
        p = self.params
        if p.useCats:
            gt = self._gts[vidId,catId]
            dt = self._dts[vidId,catId]
        ...
        d_b = [d['boundary_list'] for d in dt]
        ...
        mask_ious = np.zeros([len(d_m), len(g_m)])
        for i, j in np.ndindex(mask_ious.shape):
            mask_ious[i, j] = iou_seq(d_m[i], g_m[j])
        
        boundary_ious = np.zeros([len(d_b), len(g_b)])
        for i, j in np.ndindex(boundary_ious.shape):
            boundary_ious[i, j] = iou_seq(d_b[i], g_b[j])
